...
public class TapSmithyOrnamentAdvancement : MonoBehaviour
{
    [SerializeField] private GameDatabase.TableDB.Equipment _nowSelectEquipMat0; // 승급 메인 장신구 
    ...
    public GameDatabase.TableDB.Equipment GetNowSelectEquipMat0() => _nowSelectEquipMat0;
    ...
    [SerializeField] AcSyntInfo acSyntInfo_Mat0;
    ...
    public async void SetDatas(GameDatabase.TableDB.Equipment _eqDB, bool init = false, int init_mat = -1)
    {
        ...
        if (init)
        {
            ...
            acSyntInfo_Result.txt_SuccessOrFail.text = "장신구 승급";
            ...
            {
                lastAddMat = "empty";
                _nowSelectEquipMat0 = default;
                _nowSelectEquipMat1 = default;
                _nowSelectEquipMat2 = default;
                _nowResultEquip = default;
                acSyntInfo_Mat0.img_RatingBg.sprite = SpriteAtlasMng.GetInstance().GetSpriteRatingBg(0);
                ...
                acSyntInfo_Mat0.go_Root.SetActive(false);
        ...
        {
            if (_nowSelectEquipMat0.aInUid > 0 && _nowSelectEquipMat1.aInUid > 0 && _nowSelectEquipMat2.aInUid > 0)
            {
                if (string.Equals(lastAddMat, "mat0"))
                    _nowSelectEquipMat0 = default;
            ...
            if (_nowSelectEquipMat0.aInUid <= 0) // 승급 장신구 정보 입력 
            {
                lastAddMat = "mat0";
                _nowSelectEquipMat0 = _eqDB;
                acSyntInfo_Mat0.go_Root.SetActive(true);

                // ----- 재료1 장신구의 아이콘 정보 -----
                acSyntInfo_Mat0.img_Icon.sprite = SpriteAtlasMng.GetInstance().GetSpriteEquip(_eqDB.eq_ty, _eqDB.eq_rt, _eqDB.eq_id);
                acSyntInfo_Mat0.img_RatingLine.color = ResourceDatabase.GetInstance().GetItemColor(_eqDB.eq_rt);
                acSyntInfo_Mat0.img_RatingBg.sprite = SpriteAtlasMng.GetInstance().GetSpriteRatingBg(_eqDB.eq_rt);
                acSyntInfo_Mat0.txt_Rating.color = ResourceDatabase.GetInstance().GetItemColor(_eqDB.eq_rt);
                acSyntInfo_Mat0.txt_Rating.text = _eqDB.eq_rt > 0 ? LanguageGameData.GetInstance().GetString(string.Format("item.rating.string.{0}", _eqDB.eq_rt)) : "";

                acSyntInfo_Mat0.txt_EhntLv.text = string.Format("Lv.{0}", _eqDB.m_ehnt_lv); // 장비 강화 레벨 
                acSyntInfo_Mat0.txt_EquipName.text = GameDatabase.StringFormat.GetEquipName(_eqDB.eq_ty, _eqDB.eq_rt, _eqDB.eq_id); // 장비 이름 
            }
            else if (_nowSelectEquipMat0.aInUid > 0 && _nowSelectEquipMat1.aInUid <= 0) // 재료 1 정보 입력 
            {
                lastAddMat = "mat1";
            ...
            else if (_nowSelectEquipMat0.aInUid > 0 && _nowSelectEquipMat1.aInUid > 0 && _nowSelectEquipMat2.aInUid <= 0)  // 재료 2 정보 입력 
            {
                lastAddMat = "mat2";
            ...
            if (_nowSelectEquipMat0.aInUid > 0 && _nowSelectEquipMat1.aInUid > 0 && _nowSelectEquipMat2.aInUid > 0)
            {
                ...
                float dft_prob = GameDatabase.GetInstance().smithyDB.GetAcceAdvcRate(); // 기본 성공률 
                float bns_prob = GameDatabase.GetInstance().tableDB.GetUserInfo().m_ac_advc_rt7_p; // 실패 보너스 성공률
                ...
                acSyntInfo_Result.txBlueDiaPrice.text = tbc_dia + goods_db.m_dia >= tbc_dia_price * 5 ? string.Format("{0:#,0}", tbc_dia_price * 5) : string.Format("<color=red>{0:#,0}</color>", tbc_dia_price * 5); // 강화 필요 블루 다이아 
                ...
                acSyntInfo_Result.imBlueDiaStartBtnBg.sprite = SpriteAtlasMng.GetInstance().GetSpriteButtonRedOrGray(goods_db.m_gold >= gold_price * 3 && tbc_dia + goods_db.m_dia >= tbc_dia_price * 5);
    ...
    int fail_r_ac_advc_result_cnt = 0;
    // 고대 -> 전설로 승급 시작 (TBC) 
    public void Click_StartAdvcRedDia() => StartAdvc(true);
    // 고대 -> 전설로 승급 시작 (Blue Dia) 
    public void Click_StartAdvcBlueDia() => StartAdvc(false);
    // 시작 
    public async void StartAdvc(bool isTbc)
    {
        Loading.Full(false);
        var mat0_equip = _nowSelectEquipMat0;
        var mat1_equip = _nowSelectEquipMat1;
        var mat2_equip = _nowSelectEquipMat2;

        // 장신구 승급은 고대 등급부터 가능
        if (mat0_equip.eq_rt < 6 || mat1_equip.eq_rt < 6 || mat2_equip.eq_rt < 6)
        {
            PopUpMng.GetInstance().popupNotice.OpenCloseConmfirmNoticeBox(string.Format("장신구 승급은 {0} 등급부터 가능합니다.", GameDatabase.StringFormat.GetRatingColorText(6)));
            SetDatas(default, true, -1);
            MainUI.GetInstance().tapSmithy.initOnStartSmithy.SetInit();
            return;
        }
        
        long dft_gold_price = GameDatabase.GetInstance().questDB.GetQuestEquipAcceSyntAdvcGold(mat0_equip.eq_rt, mat0_equip.eq_id);
        long gold_price = isTbc ? dft_gold_price : dft_gold_price * 3;
        int dft_dia_price = GameDatabase.GetInstance().questDB.GetQuestEquipAcceSyntAdvcTBCDia(mat0_equip.eq_rt, mat0_equip.eq_id);
        int dia_price = isTbc ? dft_dia_price : dft_dia_price * 5;
        var goods_db = GameDatabase.GetInstance().tableDB.GetTableDB_Goods();
        ...
        if (goods_db.m_gold >= gold_price && isOkDia == true)
        {
            bool isError = false;
            if (isTbc == true) // TBC 
            {
            ...
            if (!isError)
            {
                ...
                string random_probability = GameDatabase.GetInstance().chartProbabilityDB.GetSelectedProbabilityFileId(string.Format("r_ac_synt_p{0}", uInfo.m_ac_advc_rt7_p < 15 ? (ObscuredInt)15 : uInfo.m_ac_advc_rt7_p > 100 ? (ObscuredInt)100 : uInfo.m_ac_advc_rt7_p));
                ...
                if (bro1.IsSuccess())  // 합성 결과 
                {
                    ...
                    if (isSyntSuccess)
                    {
                        _nowResultEquip = _nowSelectEquipMat0;
                        _nowResultEquip.eq_rt++;
                        _nowResultEquip.eq_id = GameDatabase.GetInstance().GetRandomPercent() < 25 ? 2 : 1;

                        if (string.IsNullOrEmpty(_nowResultEquip.indate)) // insert or change 
                        {
                            long unused_uid = GameDatabase.GetInstance().tableDB.GetUnusedUID(mat0_equip.aInUid);
                            string recycle_idate = GameDatabase.GetInstance().tableDB.GetUIDSearchToInDate(unused_uid);
                            if (string.IsNullOrEmpty(recycle_idate)) // insert 
                            {
                                Task<bool> tsk_insert = GameDatabase.GetInstance().tableDB.SendDataEquipment(_nowResultEquip, "insert");
                                while (tsk_insert.IsCompleted == false) { await Task.Delay(100); }
                            }
                            else
                            {
                                _nowResultEquip.indate = recycle_idate;
                                Task<bool> tsk_change = GameDatabase.GetInstance().tableDB.SendDataEquipment(_nowResultEquip, "change");
                                while (tsk_change.IsCompleted == false) { await Task.Delay(100); }
                        ...
                        {
                            Param pam_update = new Param();
                            pam_update.Add("eq_rt", _nowResultEquip.eq_rt);
                            pam_update.Add("eq_id", _nowResultEquip.eq_id);
                            Task<bool> tsk_update = GameDatabase.GetInstance().tableDB.SendDataEquipment(_nowResultEquip, "update", pam_update);
                            while (tsk_update.IsCompleted == false) { await Task.Delay(100); }
                        }

                        uInfo.m_ac_advc_rt7_p = 0;
                        uInfo.m_ac_advc_succ_cnt++;
                        GameDatabase.GetInstance().achievementsDB.ASetInCount(GameDatabase.AchievementsDB.Nbr.nbr22, 1, true, true); // 업적, nbr8 장신구 합성 성공! 
                    ...
                    {
                        uInfo.m_ac_advc_rt7_p += isTbc ? GameDatabase.GetInstance().chartDB.GetDicBalance("ac.red.dia.rt6.synt.failled.bns").val_int : GameDatabase.GetInstance().chartDB.GetDicBalance("ac.blue.dia.rt6.synt.failled.bns").val_int;
                        uInfo.m_ac_advc_fail_cnt++;
                        _nowResultEquip = new GameDatabase.TableDB.Equipment() { eq_ty = mat0_equip.eq_ty, eq_rt = mat0_equip.eq_rt, eq_id = mat0_equip.eq_id };
                        GameDatabase.GetInstance().achievementsDB.ASetInCount(GameDatabase.AchievementsDB.Nbr.nbr23, 1, true, true); // 업적, nbr9 장신구 합성 실패 
                    }
                    #endregion
                ...
                else
                {
                    ...
                    if (fail_r_ac_advc_result_cnt < 3)
                    {
                        StartAdvc(isTbc);
                        fail_r_ac_advc_result_cnt++;
                    }
                    else
                    {
                        SetDatas(default, true, -1);
    ...
    // 승급 장신구 해제 
    public void Click_ReleaseMat0()
    {
        if (!isSyntWait && _nowSelectEquipMat0.aInUid > 0)
        {
            //foreach (var item in GameObject.FindObjectsOfType<ScrollIndexCallbackSmithy>())
            //    item.RefreshSelectReleaseAcceAdvancement(_nowSelectEquipMat0);
    ...
    public void Click_ReleaseMat1()
    {
        ...
        {
            foreach (var item in GameObject.FindObjectsOfType<ScrollIndexCallbackSmithy>())
                item.RefreshSelectReleaseAcceAdvancement(_nowSelectEquipMat1);
    ...
    public void Click_ReleaseMat2()
    {
        ...
        {
            foreach (var item in GameObject.FindObjectsOfType<ScrollIndexCallbackSmithy>())
                item.RefreshSelectReleaseAcceAdvancement(_nowSelectEquipMat2);
