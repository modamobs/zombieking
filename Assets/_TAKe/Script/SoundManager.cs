...
public class SoundManager : MonoSingleton<SoundManager>
{
    ...
    void SetupBGM()
    {
        ...
        bgmPlayer.loop = true;
        bgmPlayer.volume = GameDatabase.GetInstance().option.setting_bgm_snd == 0 ? masterVolumeBGM: 0;
        bgmPlayer.Play();
    }

    // 한 번 재생 : 볼륨 매개변수로 지정
    public void PlaySound(string a_name, float a_volume = 0.5f)
    {
        ...
        bool isOn = GameDatabase.GetInstance().option.setting_sfx_snd == 0;
        float vol = isOn == true ? a_volume * masterVolumeSFX : 0;
        sfxPlayer.PlayOneShot(audioClipsDic[a_name], vol);
    }
